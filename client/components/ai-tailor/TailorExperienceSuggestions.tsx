...
import { Briefcase, Check, X, Tag, HelpCircle, RotateCcw } from "lucide-react";
import type { TailorExperienceSuggestion } from "@/services/ai.service";
import type { Experience } from "@/types/resume";

interface TailorExperienceSuggestionsProps {
  suggestions: TailorExperienceSuggestion[];
  experiences: Experience[];
  decisions: Record<string, "accepted" | "rejected" | "pending">;
  onDecision: (key: string, decision: "accepted" | "rejected" | "pending") => void;
}

export default function TailorExperienceSuggestions({
  suggestions,
  experiences,
  decisions,
  onDecision,
}: TailorExperienceSuggestionsProps) {
  if (!suggestions || suggestions.length === 0) {
    return null;
  }

  // Group suggestions by experienceId
  const getExperienceTitle = (expId: string) => {
    const found = experiences.find((e) => e.id === expId);
    if (found) {
      return `${found.jobTitle || "Role"} at ${found.company || "Company"}`;
    }
    ...
    <div className="bg-white rounded-2xl border border-gray-200 shadow-xs p-6 md:p-8 space-y-6">
      <div className="flex items-center justify-between border-b border-gray-100 pb-4">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="text-base font-bold text-gray-900">Work Experience Rewrites</h3>
            <p className="text-xs text-gray-500">
              Strengthen active verbs and highlight job-specific keywords without adding unheld responsibilities.
            </p>
          </div>
        </div>
      ...
      <div className="space-y-5">
        {suggestions.map((item, idx) => {
          const itemKey = `exp-${item.experienceId}-${item.bulletIndex}`;
          const status = decisions[itemKey] || "pending";
          const expHeader = getExperienceTitle(item.experienceId);
